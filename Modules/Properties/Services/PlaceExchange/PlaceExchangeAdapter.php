...
class PlaceExchangeAdapter extends InventoryAdapter {
    ...
    protected function fillAdUnit(AdUnit $adUnit, BroadcastLocation $location, ProductResource $product, array $context, float $impressionsShare): void {
        ...
        $name = $location->external_id->external_id;
        ...
        $adUnit->asset            = new AdUnitAsset(
            ...
            mimes       : $mimeTypes,
            name        : $name,
            screen_count: max(1, $location->screen_count),
...
//			bidfloor   : round($product->programmatic_price / $this->getConfig()->conversion_rate_usd_to_cad),
        ...
        $adUnit->eids             = [
            new AdUnitExternalId(source: "broadsign.com",
                                 uids  : [["id" => $location->external_id->external_id]],
            ),
            new AdUnitExternalId(source: "neo-ooh.com",
                                 uids  : [["id" => (string)$location->id]],
            ),
        ];
...
//			base_rate: round($product->programmatic_price / $this->getConfig()->conversion_rate_usd_to_cad),
    ...
    public function createProduct(ProductResource $product, array $context): InventoryResourceId|null {
        ...
        foreach ($product->broadcastLocations as $broadcastLocation) {
            if ($screensCount === 0 || $broadcastLocation->screen_count === 0) {
                // No screen ? That's weird. Do not replicate a location that has no screen.
                continue;
            }
            ...
            $adUnit->name   = $broadcastLocation->external_id->external_id;
            $adUnit->slot   = new AdUnitSlot(
                h           : $product->screen_height_px,
                max_duration: $product->loop_configuration->spot_length_ms / 1_000,
                min_duration: min(5, $product->loop_configuration->spot_length_ms / 1_000),
            ...
            try {
                $this->fillAdUnit($adUnit, $broadcastLocation, $product, $context, $impressionsShare);
                $adUnit->save();
    ...
    public function updateProduct(InventoryResourceId $productId, ProductResource $product): InventoryResourceId|false {
        ...
        foreach ($product->broadcastLocations as $broadcastLocation) {
            ...
            if (!isset($productId->context["units"][$broadcastLocation->id])) {
                ...
                $adUnit = AdUnit::find($client, $productId->context["units"][$broadcastLocation->id]["id"]);
            }

            $unitName = $adUnit->name;

            $this->fillAdUnit($adUnit, $broadcastLocation, $product, $productId->context, $impressionsShare);
        ...
        // We now want to compare the list of adunits we just built against the one we were given.
        // Any adunit listed in the latter but missing in the former will have to be removed
