...
class FillMissingTrafficValueJob implements ShouldQueue {
	...
	public function handle() {
		...
		foreach ($properties as $property) {
			...
			if ($property->traffic->data->first(fn(MonthlyTrafficDatum $t) => $t->year === $currentYear && $t->month === $currentMonth)) {
				// ignore
			...
			if ($property->traffic->missing_value_strategy === "USE_PLACEHOLDER") {
				...
				MonthlyTrafficDatum::query()->create([
			...
			/** @var ?MonthlyTrafficDatum $prevRecord */
			$prevRecord = $property->traffic->data->first(fn(MonthlyTrafficDatum $t) => $t->year === 2019 && $t->month === $currentMonth);
