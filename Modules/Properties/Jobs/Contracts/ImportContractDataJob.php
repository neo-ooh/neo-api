...
use JsonException;
use Neo\Modules\Properties\Jobs\Contracts\Structs\FlightDefinition;
use Neo\Modules\Properties\Jobs\Contracts\Structs\LineDefinition;
...
/**
 * Synchronize the state of a contract in Connect against the associated external contract.
 * This job imports and update all the lines in the contract, as well as the associated plan if available.
 */
class ImportContractDataJob implements ShouldQueue {
	...
	public function handle(): void {
		...
		// If the contract got updated, we want to preserve as many associations as possible.
	...
	public function getFlightsFromPlan(Contract $contract, ConsoleOutput $output): Collection {
		...
		foreach ($plan->flights as $i => $flight) {
			$flightDefinition = new FlightDefinition(
				...
				uid       : $flight->id,
				type      : $flight->type,
				start_date: Carbon::parse($flight->start_date)->toDateString(),
				end_date  : Carbon::parse($flight->end_date)->toDateString(),
			);
