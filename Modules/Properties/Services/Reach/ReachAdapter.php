...
//        $screen->floor_cpm                    = $product->price;
        ...
        $weeklyImpressions                    = collect($product->weekdays_spot_impressions)
            ->map(fn($spotImpressions, $i) => ($product->operating_hours[$i]?->open_length_min / ($product->loop_configuration->loop_length_ms / 60_000 /*ms to min*/)) * $product->loop_configuration->spotsCount()
            )
            ->sum();
