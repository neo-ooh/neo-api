...
class Product extends Model implements WithImpressionsModels, WithAttachments {
    ...
    */
     ...
     *
     * @param $weekTraffic
     * @return array A 1-indexed array of the amount of impressions generated on each day for one spot on one play
     */
    public function getSpotImpressionsForWeek($weekTraffic) {
        ...
        foreach (range(1, 7) as $weekday) {
            ...
            $openingHours[$weekday] = $openLengthHours;
        }


        $hourTraffic = $weekTraffic / collect($openingHours)->values()->sum();
        ...
        foreach ($openingHours as $weekday => $openLength) {
            $spotImpressionsPerPlay[$weekday] = ($spotImpressionsPerHour * $openLength) / ($loopConfiguration->loop_length_ms / (1_000 * 60));
        }
