...
class ResourceFactory {
	public static function makeIdentifiableProduct(Product $product, OdooClient $client, OdooConfig $config) {
		$property = $product->shopping_center_id ? Property::get($client, $product->shopping_center_id[0]) : null;

		$geolocation = null;

		if ($property && ((int)round($property->partner_longitude) !== 0 || (int)round($property->partner_latitude) !== 0)) {
		...
		return new IdentifiableProduct(
			...
			product   : new ProductResource(
				            ...
				            allows_audio             : true,
				            allows_motion            : true,
				            property_id              : $property ? new InventoryResourceId(
					                                       inventory_id: $config->inventoryID,
					                                       external_id : $property->getKey(),
					                                       type        : InventoryResourceType::Property
				                                       ) : null,
				            property_name            : $property ? trim($property->name) : '-',
				            property_type            : null,
				            address                  : $property ? $property->getAddress() : null,
				            geolocation              : $property ? $geolocation : null,
				            timezone                 : null,
				            operating_hours          : null,
				            weekly_traffic           : $property ? (int)ceil(($property->annual_traffic / 365) * 7) : 0,
