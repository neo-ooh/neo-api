...
class ImportContractDataFromDocumentJob implements ShouldQueue {
    ...
    public function handle() {
        ...
        $contract->start_date = $this->order->getGuaranteedOrders()->min(fn($date) => Carbon::parse($date->date_start));
        $contract->end_date = $this->order->getGuaranteedOrders()->max(fn($date) => Carbon::parse($date->date_end));
        $contract->save();
